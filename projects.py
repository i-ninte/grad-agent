...
def _load_catalog_extras() -> list[dict]:
    if not _CATALOG_PATH.exists(): return []
    try:
...
def _all_projects() -> list[dict]:
    """Seed list plus anything synced from GitHub / HF / local. Dedup by name."""
    seen: dict[str, dict] = {}
    for p in SEED_PROJECTS + _load_catalog_extras():
        key = (p.get("name") or "").strip().lower()
        if not key or not p.get("pitch"): continue
        # First entry wins; seed takes precedence because it's iterated first
